#!/bin/bash
# Seed KV with initial configuration for the dev environment.
# Run from the project root: bash scripts/seed-kv-dev.sh
#
# Prerequisites: the deeprecall-config-dev KV namespace and the default D1
...
: "${KV_ID:?KV_ID is required (id of the deeprecall-config-dev KV namespace)}"
: "${D1_DATABASE_ID:?D1_DATABASE_ID is required (id of the deeprecall-db-default-dev D1 database)}"

echo "Seeding KV namespace: deeprecall-config-dev ($KV_ID)"

# Generate a product API key for dev.
# Note: the admin key is a worker secret, not KV — set it separately via
# scripts/setup-admin-key.sh (see docs/ADMIN_GUIDE.md).
API_KEY="dev-api-key-$(openssl rand -hex 16)"
...
echo ""
echo "=== Product API Key (save this — it is NOT recoverable later!) ==="
...
pnpx wrangler kv key put --namespace-id="$KV_ID" --remote "product:default:config" '{"product_id":"default","name":"default","db_id":"'"$D1_DATABASE_ID"'","db_name":"deeprecall-db-default-dev","vectorize_name":"deeprecall-vectors-default-dev","policyOverrides":{},"features":{}}'
...
echo ""
echo "Add the product API key to your Postman environment or .dev.vars:"
echo "  X-API-Key: $API_KEY"
echo ""
echo "If this is the first time setting up dev, also set the admin key secret:"
echo "  bash scripts/setup-admin-key.sh dev"
